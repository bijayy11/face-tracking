...
def calculate_dominance_score(face_area, eye_stability, mouth_movement):
    ...
    dominance_score = normalized_face_area * 0.5 + eye_stability * 0.2 + mouth_movement * 0.3
...
def detect_and_track_faces():
    ...
    with mp_face_detection.FaceDetection(min_detection_confidence=0.8) as face_detection, \
         mp_face_mesh.FaceMesh(static_image_mode=False, max_num_faces=10, min_detection_confidence=0.8) as face_mesh:
        ...
        while True:
            ...
            if face_results.detections:
                for detection in face_results.detections:
                    ...
                    # Display Eye Stability and Mouth Movement for each face
                    cv2.putText(frame, f"Eye Stability: {eye_stability:.2f}", (x, y + h + 20),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
                    cv2.putText(frame, f"Mouth Movement: {mouth_movement:.2f}", (x, y + h + 40),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
