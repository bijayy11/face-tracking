...
def calculate_rotation_angles(target_position, frame_center):
    """Calculate rotation angles based on target position and frame center."""
    dx = target_position[0] - frame_center[0]
    dy = target_position[1] - frame_center[1]

    # Calculate horizontal and vertical angles
    horizontal_angle = math.degrees(math.atan2(dx, frame_center[0]))
    vertical_angle = math.degrees(math.atan2(dy, frame_center[1]))

    return int(horizontal_angle), int(vertical_angle)

def send_coordinates_to_cpp(x, y, server_ip='127.0.0.1', port=65432):
    """Send coordinates to the C++ program via socket."""
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.connect((server_ip, port))
            message = f"{x},{y}".encode()
            s.sendall(message)
            print(f"Sent angles: ({x}, {y})")
...
def calculate_dominance_score(face_area, eye_stability, mouth_movement):
    ...
    dominance_score = normalized_face_area * 0.4 + eye_stability * 0.2 + mouth_movement * 0.4
...
def detect_and_track_faces():
    ...
    with mp_face_detection.FaceDetection(min_detection_confidence=0.5) as face_detection, \
        ...
        try:
            ...
            frame_center = (320, 240)  # Example frame center, adjust as necessary
            while True:
                ret, frame = cap.read()
                if not ret:
                ...
                if face_results.detections:
                    for detection in face_results.detections:
                        ...
                        if dominance_score > dominant_score + 0.5:
                            dominant_score = dominance_score
                            dominant_face = face_center
                            dominant_coords = (x, y, w, h)

                    # Send angles for dominant face to align it at the frame center
                    current_time = time.time()
                    if dominant_face and current_time - last_sent_time >= 1:
                        x_angle, y_angle = calculate_rotation_angles(dominant_face, frame_center)
                        send_coordinates_to_cpp(x_angle, y_angle)
                        ...
                        # Print angles and score to the terminal
                        print(f"Angles to Align: ({x_angle}, {y_angle}), Dominance Score: {dominant_score:.2f}")
